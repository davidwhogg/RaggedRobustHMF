...
We will refer to the $nj$ element of $Y$ as $y_{nj}$ with $1\leq n\leq N$ and $1\leq j\leq d$.

In PCA, all of these $N\times d$ data points $y_{nj}$ is treated as identically valuable;
...
\begin{align}
    y_{nj} &= \sum_{k=1}^K a_{nk} \sum_{m=-M}^M b_{k,m}\,\exp(i m \frac{\lambda_{nj}}{L}) + \text{residual}_{nj} ~,
\end{align}
where we have (arbitrarily) chosen to work in a Fourier-series basis,
